...
- **Claude-powered engine** *(optional, `ANTHROPIC_API_KEY`)* — adds real
  translation and handles messy, code-mixed, or idiomatic phrasing the
  lexicon misses. If it fails for any reason — no key, rate limit, network —
  the system falls back to the offline engine automatically. The demo never
  breaks because a model call didn't come back.
...
│   └── llm.py               Claude-powered engine (optional upgrade)
├── engine/               The analytical core
